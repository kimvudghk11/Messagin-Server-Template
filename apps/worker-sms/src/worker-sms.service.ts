...
@Injectable()
export class WorkerSmsService implements OnModuleInit, OnModuleDestroy {
  private readonly logger = new Logger(WorkerSmsService.name);
  ...
  constructor(
    ...
    const kafka = new Kafka({
      clientId: this.configService.get<string>('KAFKA_WORKER_SMS_CLIENT_ID', 'worker-sms'),
      brokers,
    });

    this.consumer = kafka.consumer({
      groupId: this.configService.get<string>('KAFKA_WORKER_SMS_GROUP_ID', 'worker-sms-group'),
    });
  }
  ...
  async onModuleInit(): Promise<void> {
    ...
    await this.consumer.run({
      eachMessage: async ({ message }) => {
        ...
        try {
          ...
          if (event.channel !== ChannelType.SMS) return;
    ...
    this.logger.log(`worker-sms consumer started: topic=${topic}`);
  }
  ...
  private async process(event: MessageSendEvent): Promise<void> {
    ...
    const existingSuccess = await this.messageDispatchRepository.findOne({
      where: { messageRequestId: event.messageRequestId, recipientId: event.recipientId, channelType: ChannelType.SMS, status: MessageDispatchStatus.SUCCESS },
    ...
    const dispatch = this.messageDispatchRepository.create({
      ...
      providerType: ProviderType.SMS_VENDOR,
    ...
    try {
      const providerMessageId = `sms_${randomUUID()}`;
      ...
      if (canRetry) {
        ...
        savedDispatch.lastErrorCode = 'SMS_SEND_FAILED';
  ...
  private async writeLog(dispatchId: string, logType: DispatchLogType, status: DispatchLogStatus, response: Record<string, unknown>): Promise<void> {
    await this.messageDispatchLogRepository.save(
      this.messageDispatchLogRepository.create({ dispatchId, logType, status, providerCode: 'SMS_VENDOR', providerMessage: null, rawRequest: null, rawResponse: response, loggedAt: new Date() }),
    );
  }
}
