...
@Injectable()
export class WorkerKakaoService implements OnModuleInit, OnModuleDestroy {
  private readonly logger = new Logger(WorkerKakaoService.name);
  ...
  constructor(
    ...
    const kafka = new Kafka({
      clientId: this.configService.get<string>('KAFKA_WORKER_KAKAO_CLIENT_ID', 'worker-kakao'),
      brokers,
    });

    this.consumer = kafka.consumer({
      groupId: this.configService.get<string>('KAFKA_WORKER_KAKAO_GROUP_ID', 'worker-kakao-group'),
    });
  }
  ...
  async onModuleInit(): Promise<void> {
    ...
    await this.consumer.run({
      eachMessage: async ({ message }) => {
        ...
        try {
          ...
          if (event.channel !== ChannelType.KAKAO) return;
    ...
    this.logger.log(`worker-kakao consumer started: topic=${topic}`);
  }
  ...
  private async process(event: MessageSendEvent): Promise<void> {
    ...
    const existingSuccess = await this.messageDispatchRepository.findOne({
      where: { messageRequestId: event.messageRequestId, recipientId: event.recipientId, channelType: ChannelType.KAKAO, status: MessageDispatchStatus.SUCCESS },
    ...
    const dispatch = this.messageDispatchRepository.create({
      ...
      providerType: ProviderType.KAKAO_VENDOR,
    ...
    try {
      const providerMessageId = `kakao_${randomUUID()}`;
      ...
      if (canRetry) {
        ...
        savedDispatch.lastErrorCode = 'KAKAO_SEND_FAILED';
        ...
        await this.writeLog(savedDispatch.id, DispatchLogType.RETRY, DispatchLogStatus.FAILED, {
          error: errorMessage, nextRetryAt: savedDispatch.nextRetryAt, retryCount: savedDispatch.retryCount,
        });
      } else {
  ...
  private async writeLog(dispatchId: string, logType: DispatchLogType, status: DispatchLogStatus, response: Record<string, unknown>): Promise<void> {
    await this.messageDispatchLogRepository.save(
      this.messageDispatchLogRepository.create({ dispatchId, logType, status, providerCode: 'KAKAO_VENDOR', providerMessage: null, rawRequest: null, rawResponse: response, loggedAt: new Date() }),
    );
  }
}
